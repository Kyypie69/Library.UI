...
local liveConnections = {} -- Track live label connections for cleanup
...
-- // UTILITY FUNCTIONS
local function FormatPadding(str, padLeft, padRight)
    padLeft = padLeft or 0
    padRight = padRight or 0
...
function KyyfiiiLibrary:CreateWindow(Title, Description)
    ...
    function Tabs:CreateTab(Title, Icon)
        ...
        -- // LIVE LABEL - Auto-updating text with real-time data binding
        function Elements:CreateLiveLabel(Title, ValueFunction, UpdateInterval)
            UpdateInterval = UpdateInterval or 0.1 -- Default update every 0.1 seconds for smooth updates
            
            local Label = Instance.new("Frame")
            local corner = Instance.new("UICorner")
            local LabelTitle = Instance.new("TextLabel")
            local ValueLabel = Instance.new("TextLabel") -- Separate value display
            ...
            ValueLabel.Text = "<b>...</b>"
            ValueLabel.TextColor3 = PALETTE.Accent -- White accent for the value
            ...
            if UpdateInterval <= 0.05 then
                -- Use RenderStepped for very fast updates (animations, FPS, etc)
                connection = runService.RenderStepped:Connect(function()
                    updateValue()
                end)
            else
                -- Use heartbeat for slower updates (saves performance)
            ...
            return {
                ...
                UpdateInterval = function(self, newInterval)
                    -- Note: Changing interval requires recreating connection (simplified here)
                    UpdateInterval = newInterval
                end
            }
        end

        -- // DYNAMIC LABEL - One-time text updates (original label enhanced)
...
-- // UTILITY: Create live notification (different from static notification)
function KyyfiiiLibrary:createLiveNotification(descFunction, duration, updateInterval)
    updateInterval = updateInterval or 0.5
    
    local Notification = Instance.new("Frame")
    -- [Same notification setup as before...]
    local bg = Instance.new("ImageLabel")
    local corner1 = Instance.new("UICorner")
    local corner2 = Instance.new("UICorner")
    local title = Instance.new("TextLabel")
    ...
    Notification.Name = "LiveNotification"
    Notification.Parent = Library
    Notification.AnchorPoint = Vector2.new(1,1)
    Notification.BackgroundColor3 = PALETTE.Background
    Notification.BackgroundTransparency = .1
    ...
    title.Font = Enum.Font.Jura
    title.RichText = true
    title.Text = "<b>NavyPurple  |</b>  Live Notification"
    title.TextColor3 = PALETTE.TextPrimary
    title.TextSize = 15
    title.TextXAlignment = Enum.TextXAlignment.Left
    Instance.new("UIPadding", title).PaddingLeft = UDim.new(0,10)
    ...
    connection = runService.Heartbeat:Connect(function()
        if tick() - startTime >= (duration or 5) then
            connection:Disconnect()
            local tOut = TweenService:Create(Notification, TweenInfo.new(.5),
                {Position = UDim2.new(1,300,1,targetY)})
            tOut:Play()
        ...
        local success, result = pcall(descFunction)
        if success then
            descLbl.Text = tostring(result)
        end
    end)
end
